...
contract OrganizationRegistry {
  ...
  RegistryManager internal immutable rm;
  ...
  constructor(UserRegistry _user_registry, RegistryManager _rm) {
  ...
  modifier only_organization_owner(uint256 orgId) {
    ...
    require(found, InvalidIdentity());
    _;
  }

  modifier only_registry() {
    require(msg.sender == address(rm.oid_registry()), NotRegistry());
    _;
  }
   ...
   */
  function create_first_level_organization(
    uint256 orgId,
   ...
   */
  function add_managed_identity(
    uint256 orgId,
    uint256 userId
  ) external only_organization_owner(orgId) {
   ...
   * @param userId The unique identifier of the user.
   */
  function remove_managed_identity(
    uint256 orgId,
    uint256 userId
  ) external only_organization_owner(orgId) {
   ...
   */
  function validate_second_level_org_creation(
    uint256 orgId,
    Identity memory creator
  ) internal view {
    ...
    require(
      org.owner.public_keys[0].key == creator.public_keys[0].key || found,
      InvalidAssociation()
    );
  }
}
