...
import {Identity, PublicKey, MAX_PUBLIC_KEYS_LENGTH, SIG_V_S} from "@~/library/Structs.sol";
import {RegistryManager} from "../RegistryManager.sol";
import {RcEcdsa, ECDSA} from "../utils/RcEcdsa.sol";
import {Nonce} from "../utils/Nonce.sol";
import "../library/Errors.sol";

contract UserRegistry is Nonce, RcEcdsa {
  /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/
  ...
  constructor(RegistryManager _rm) {
    rm = _rm;
  }
   ...
   */
  function recover_oid(
    uint256 oid,
    uint64 deadline,
    PublicKey memory new_key,
    ...
    require(
      is_valid_recovery_signature(oid, nonce, deadline, recovery_addresses, signature) == SIG_V_S,
  ...
  function add_public_key(uint256 _oid, address _key, address _sigVerifier) public {
    ...
    rm.oid_registry().link_address(_oid, _key);
  ...
  function remove_public_key(uint256 _oid, uint256 _keyIndex) public {
    ...
    rm.oid_registry().unlink_address(key);
  ...
  function requires_is_oid_owner(uint256 _oid, address caller) internal view {
    uint256 oid = rm.oid_registry().get_oid(caller);
